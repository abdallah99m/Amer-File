...
namespace AmerPay.Controllers
{
    public class RequestController : Controller
    {
        ...
        public IActionResult Create(Request obj,IFormFile file)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (file != null)
                {
                    ...
                    obj.FileForward = @"wwwroot\Files\" + filename + extension;
                    ...
                    using (Aes encryptor = Aes.Create())
                    {
                        ...
                        using (FileStream fsOutput = new FileStream(Path.Combine(uploads, filename+"abdallah" + extension), FileMode.Create))
                        {
                            ...
                            {
                                using (FileStream fsInput = new FileStream(obj.FileForward, FileMode.Open))
                                {
                                    int data;
                    ...
                    if (obj.FileForward != null)
                    {
                        var oldImagePath = Path.Combine(wwwRootPath, obj.FileForward.TrimStart('\\'));
                    ...
                    obj.FileForward = @"wwwroot\Files\" + filename + "abdallah" + extension;
        ...
        public IActionResult Edit(int id)
        {
            ...
            using (Aes encryptor = Aes.Create())
            {
                ...
                encryptor.Key = pdb.GetBytes(32);
                encryptor.IV = pdb.GetBytes(16);
                using (FileStream fsInput = new FileStream(_db.requests.Find(id).FileForward, FileMode.Open))
                {
                    using (CryptoStream cs = new CryptoStream(fsInput, encryptor.CreateDecryptor(), CryptoStreamMode.Read))
                    {
                        using (FileStream fsOutput = new FileStream(Path.Combine(uploads, _db.requests.Find(id).FileName+"abdallah15" + _db.requests.Find(id).FileExtension), FileMode.Create))
                        {
                            int data;
